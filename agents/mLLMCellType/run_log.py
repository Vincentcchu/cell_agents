...
# Configure logging to write to a logfile and duplicate stdout/stderr (prints + logging go to file)
import logging
import sys
import atexit
log_file = 'mllm_run.log'
# Ensure root logger writes to stderr (which we'll tee to file) with a readable formatter
...
# Simple Tee to duplicate stdout/stderr into the log file
class Tee(object):
...
# Save results
adata.write('mllm_free_models_notebook.h5ad')
